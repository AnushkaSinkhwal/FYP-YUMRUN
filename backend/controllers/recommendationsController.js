...
exports.getHealthRecommendations = async (req, res) => {
    try {
        ...
        console.log(`Getting recommendations for health condition: ${healthCondition || 'Healthy'} for user: ${userId || 'Anonymous'}`);
        ...
        try {
            ...
            if (userHealthProfile) {
                ...
                if (userHealthProfile.dietaryPreferences && userHealthProfile.dietaryPreferences.length > 0 && 
                ...
                if (userHealthProfile.healthConditions && userHealthProfile.healthConditions.length > 0 && 
                    ...
                    if (userHealthProfile.healthConditions.includes('Heart Disease') || 
                    ...
                    if (healthFilters.length > 0) {
                        query.$or = query.$or ? [...query.$or, ...healthFilters] : healthFilters;
                    }
                }
                
                // Handle allergies
                if (userHealthProfile.allergies && userHealthProfile.allergies.length > 0) {
            ...
            const formattedItems = items.map(item => ({
                ...
                healthBenefits: getHealthBenefits(healthCondition, item.healthAttributes),
                isCustomizable: item.customizationOptions?.allowAddIngredients || item.customizationOptions?.allowRemoveIngredients
            }));
...
exports.getPersonalizedRecommendations = async (req, res) => {
    try {
        ...
        const pastOrders = await Order.find({ userId }).sort({ createdAt: -1 }).limit(10);
        ...
        if (pastRestaurants.size > 0) {
            query.restaurant = { $in: Array.from(pastRestaurants).map(id => mongoose.Types.ObjectId(id)) };
        }
        
        // Exclude items the user has already ordered
        if (pastOrderItems.size > 0) {
            query._id = { $nin: Array.from(pastOrderItems).map(id => mongoose.Types.ObjectId(id)) };
        }
        
        // Apply health profile filters if available
        if (user.healthProfile) {
            // Dietary preferences
            if (user.healthProfile.dietaryPreferences && 
        ...
        const recommendedItems = await MenuItem.find(query).sort({ averageRating: -1 }).limit(8);
        ...
        return res.status(200).json({
            ...
            data: {
                recommendations: formattedItems,
                message: 'Personalized recommendations based on your past orders and preferences'
            }
        });
    } catch (error) {
...
function getFallbackRecommendations(healthCondition, userProfile = null) {
    ...
    if (userProfile && userProfile.healthConditions && 
    ...
    const fallbackData = {
        ...
        'Hypertension': [
            {
                id: 'bp-1',
                name: 'Low Sodium Stir Fry',
                description: 'Fresh vegetables with tofu in a low-sodium sauce',
                calories: 320,
                protein: 15,
                carbs: 25,
                fat: 10,
                price: 420,
                image: '/uploads/placeholders/food-placeholder.jpg',
                healthBenefits: ['Low sodium', 'Rich in potassium', 'Heart-healthy'],
                isCustomizable: true
            },
            {
                id: 'bp-2',
                name: 'DASH Diet Bowl',
                description: 'Based on the DASH diet principles with lean protein and vegetables',
                calories: 350,
                protein: 20,
                carbs: 30,
                fat: 12,
                price: 470,
                image: '/uploads/placeholders/food-placeholder.jpg',
                healthBenefits: ['BP-friendly', 'Balanced nutrition', 'Low sodium'],
                isCustomizable: true
            }
        ],
        'Obesity': [
            {
                id: 'weight-1',
                name: 'Calorie-Controlled Meal',
                description: 'Portion-controlled meal with balanced macros',
                calories: 320,
                protein: 25,
                carbs: 20,
                fat: 10,
                price: 450,
                image: '/uploads/placeholders/food-placeholder.jpg',
                healthBenefits: ['Portion controlled', 'High satiety', 'Balanced nutrition'],
                isCustomizable: true
            },
            {
                id: 'weight-2',
                name: 'High Protein Salad',
                description: 'Lean protein with fiber-rich greens',
                calories: 280,
                protein: 30,
                carbs: 15,
                fat: 8,
                price: 420,
                image: '/uploads/placeholders/food-placeholder.jpg',
                healthBenefits: ['Weight management', 'High protein', 'Low calorie'],
                isCustomizable: true
            }
        ]
    };
        ...
        if (userProfile.dietaryPreferences.includes('Vegetarian') || 
            ...
            const vegItems = [
                ...
                },
                {
                    ...
                    description: 'Chickpea hummus with fresh vegetables in a whole grain wrap',
                    calories: 310,
                    protein: 12,
                    carbs: 40,
                    fat: 10,
            ...
            return userProfile.dietaryPreferences.includes('Vegan') ? 
                vegItems.map(item => ({...item, name: item.name.replace('Plant-Based', 'Vegan')})) : 
                vegItems;
        }
    }
...
function getHealthBenefits(healthCondition, healthAttributes = {}) {
    // Default benefits
    const defaultBenefits = ['Balanced nutrition', 'Fresh ingredients', 'Quality proteins'];
